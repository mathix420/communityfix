...
export async function findSimilar<T extends { similarity: number }>(db: DB, opts: {
  table: string
  columns?: string
  embedding: number[]
  where?: SQL
...
export function computeScore(f: TrustScoreFactors): number {
  ...
  const votePts = f.totalVotesReceived > 0
  ...
  const verifiedPts = f.verifiedCredentials > 0 ? Math.min(30, 20 + (f.verifiedCredentials - 1) * 5) : 0
...
async function computeUserTrustScore(ctx: Ctx, userId: string): Promise<number> {
  ...
  const [stats, voteStats, engagementStats, verifiedStats] = await Promise.all([
    db.select({
      ...
      .groupBy(users.id),
    db.select({ totalVotes: sql<number>`COALESCE(SUM(${votes.value} * ${votes.weight}), 0)`.as('total_votes') })
      .from(votes)
      .innerJoin(issues, and(eq(votes.issueId, issues.id), eq(issues.authorId, userId), ne(issues.status, 'rejected'))),
    db.select({ votesCast: sql<number>`COUNT(*)`.as('votes_cast') }).from(votes).where(eq(votes.userId, userId)),
    db.select({ verifiedCount: sql<number>`COUNT(DISTINCT ${qualifications.id})`.as('verified_count') })
      .from(qualifications)
...
export async function updateUserTrustScore(ctx: Ctx, userId: string): Promise<number> {
  const { db } = ctx
  const current = await db.query.users.findFirst({ where: eq(users.id, userId), columns: { trustScore: true } })
  const oldScore = current?.trustScore ?? 0
  const score = await computeUserTrustScore(ctx, userId)
  await db.update(users).set({ trustScore: score, trustScoreUpdatedAt: new Date() }).where(eq(users.id, userId))
  if (score !== oldScore) {
    await createAuditLog(db, { type: 'trust_score', action: 'score_update', userId, details: { oldScore, newScore: score } })
  }
  return score
}
