...
import {
  caseStudies, issues, revisions, sdgs, tags, issueTags, issueSdgs,
...
export function prescreenRevision(ctx: Ctx, prep: RevisionPrep): Promise<RevisionPrescreen> {
  return runStep<RevisionPrescreen>(ctx.anthropic, 'revision.prescreen', {
...
export async function applyRevisionPrescreen(ctx: Ctx, prep: RevisionPrep, result: RevisionPrescreen): Promise<void> {
  const { db } = ctx
  const { revision } = prep
  ...
  const autoReject = (result.classification === 'spam' || result.classification === 'vandalism')
    && confidence >= PRESCREEN_AUTO_REJECT_CONFIDENCE

  // Re-read defensively: a human owner may already have decided in the gap.
  const current = await db.query.revisions.findFirst({ where: eq(revisions.id, revisionId), columns: { status: true } })
  ...
  if (autoReject) {
    ...
    console.log(`[review-revision] Revision ${revisionId} auto-rejected (${result.classification}, confidence: ${confidence}).`)
    return
  }

  // ok / low-confidence → leave pending; just record the AI verdict for the owner.
  await db.update(revisions)
    .set({ aiVerdict: verdict, aiConfidence: String(confidence), aiReason: result.reason, updatedAt: new Date() })
    .where(eq(revisions.id, revisionId))
  console.log(`[review-revision] Revision ${revisionId} pre-screened (${result.classification}, confidence: ${confidence}). Awaiting owner review.`)
}
