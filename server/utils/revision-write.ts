 ...
 * elsewhere.
 */
export function staleConflictFields(changes: Snapshot, baseSnapshot: Snapshot, currentSnapshot: Snapshot): string[] {
...
export async function recordRevision(entry: {
  ...
  const rows = await db.insert(revisions).values({
...
export function serializeRevision(row: RevisionWithPeople): SerializedRevision {
  return {
    ...
    decidedBy: row.decidedBy ? { id: row.decidedBy.id, name: row.decidedBy.name ?? 'Anonymous' } : null,
 ...
 * skip the email.
 */
export async function proposeRevision(userId: string, input: ProposeRevisionInput, event?: H3Event) {
  ...
  if (input.targetKind === 'issue') {
    ...
    if (!node) throw createError({ statusCode: 404, statusMessage: `Issue ${input.targetId} not found` })
    ...
    await createAuditLog({ type: 'moderation', action: 'propose', issueId: node.id, userId, reason: input.note ?? null })
    if (event) await notifyOwnersOfProposal(event, { targetKind: 'issue', nodeId: node.id, nodeLabel: node.title, revisionIssueId: node.id, revisionCaseStudyId: null, proposerId: userId, note: input.note ?? null })
    return revision
  }
  ...
  if (!node) throw createError({ statusCode: 404, statusMessage: `Case study ${input.targetId} not found` })
  ...
  await triggerModeration('revision', revision.id)
  await createAuditLog({ type: 'moderation', action: 'propose', issueId: null, userId, reason: input.note ?? null })
  if (event) {
    await notifyOwnersOfProposal(event, {
      targetKind: 'case_study',
      nodeId: node.id,
      nodeLabel: node.solution?.title ? `Case study — ${node.solution.title}` : `Case study #${node.id}`,
...
async function notifyOwnersOfProposal(event: H3Event, opts: {
  targetKind: RevisionTargetKind
  nodeId: number
  nodeLabel: string
  ...
  const [owners, proposer] = await Promise.all([
    ...
    db.query.users.findFirst({ where: eq(users.id, opts.proposerId), columns: { name: true, trustScore: true } }),
  ])
  const recipients = owners.filter(o => o.id !== opts.proposerId)
  if (recipients.length === 0) return
  await Promise.all(recipients.map(owner => sendProposalNotification(event, {
 ...
 */
export async function applyRevision(approverId: string, revision: typeof revisions.$inferSelect): Promise<Snapshot> {
  const changes = (revision.changes ?? {}) as Snapshot

  if (revision.targetKind === 'issue') {
    const patch = changesToIssuePatch(changes)
    const { issue, contentChanged, parentChanged } = await updateIssue(approverId, { id: revision.issueId!, ...patch })
 ...
 */
export async function decideRevision(userId: string, revisionId: number, action: DecideAction, reason?: string | null, event?: H3Event) {
  const db = useDB()
  const revision = await db.query.revisions.findFirst({ where: eq(revisions.id, revisionId) })
  if (!revision) throw createError({ statusCode: 404, statusMessage: `Revision ${revisionId} not found` })
  if (revision.status !== 'pending') {
    throw createError({ statusCode: 409, statusMessage: `Revision ${revisionId} is already ${revision.status}` })
  }

  const node = await loadRevisionNode(revision)
  if (!node) throw createError({ statusCode: 404, statusMessage: 'The node this revision targets no longer exists' })
  ...
  if (!role) {
    throw createError({ statusCode: 403, statusMessage: 'Only an owner or an admin can decide on this suggestion' })
  }
  ...
  if (action === 'approve') {
    ...
    const movedSinceProposed = !revision.baseUpdatedAt
      || (node.updatedAt != null && node.updatedAt.getTime() !== revision.baseUpdatedAt.getTime())
    if (movedSinceProposed) {
      const currentSnapshot = revision.targetKind === 'issue'
      ...
      if (conflicts.length > 0) {
        ...
        if (revision.proposerId && event) {
          await notifyProposerOfDecision(event, revision, 'superseded', `${conflicts.join(', ')} changed since you proposed this — please review the current version and re-submit.`)
        }
        throw createError({
          statusCode: 409,
    ...
    const claimed = await db.update(revisions).set({
      status: 'approved',
      decidedById: userId,
      decidedByRole: role,
      ...
      decidedAt: now,
      updatedAt: now,
    }).where(and(eq(revisions.id, revision.id), eq(revisions.status, 'pending'))).returning({ id: revisions.id })
    if (claimed.length === 0) {
      throw createError({ statusCode: 409, statusMessage: `Revision ${revision.id} was already decided` })
    }
    ...
    try {
      ...
      await db.update(revisions).set({
        status: 'pending', decidedById: null, decidedByRole: null, decisionReason: null, decidedAt: null, updatedAt: new Date(),
      }).where(eq(revisions.id, revision.id))
      throw err
    }
    await db.update(revisions).set({ appliedSnapshot, updatedAt: new Date() }).where(eq(revisions.id, revision.id))
  ...
  if (action === 'approve' && revision.proposerId) {
    try {
      await addNodeMember({ kind: revision.targetKind, nodeId, userId: revision.proposerId, role: 'collaborator', source: 'accepted' })
    }
    catch (err) {
  ...
  if (revision.proposerId) {
    if (event) await notifyProposerOfDecision(event, revision, action === 'approve' ? 'approved' : 'rejected', reason)
    try {
...
async function notifyProposerOfDecision(event: H3Event, revision: typeof revisions.$inferSelect, decision: 'approved' | 'rejected' | 'superseded', reason?: string | null) {
  if (!revision.proposerId) return
  const db = useDB()
  const proposer = await db.query.users.findFirst({ where: eq(users.id, revision.proposerId), columns: { email: true, name: true } })
...
export async function withdrawRevision(userId: string, revisionId: number) {
  ...
  if (revision.proposerId !== userId && !isAdmin) {
    throw createError({ statusCode: 403, statusMessage: 'Only the proposer or an admin can withdraw this suggestion' })
  }

  const now = new Date()
  await db.update(revisions).set({ status: 'withdrawn', updatedAt: now }).where(eq(revisions.id, revision.id))
  await createAuditLog({
    type: 'moderation',
    action: 'withdraw',
    issueId: revision.issueId ?? null,
...
async function loadRevisionNode(revision: typeof revisions.$inferSelect): Promise<IssueRow | CaseStudyRow | null> {
  const db = useDB()
  ...
  if (revision.targetKind === 'case_study' && revision.caseStudyId != null) {
    return (await db.query.caseStudies.findFirst({ where: eq(caseStudies.id, revision.caseStudyId) })) ?? null
  }
  return null
}
...
async function revisionNodeLabel(revision: typeof revisions.$inferSelect): Promise<string> {
  const db = useDB()
  if (revision.targetKind === 'issue' && revision.issueId != null) {
    const node = await db.query.issues.findFirst({ where: eq(issues.id, revision.issueId), columns: { title: true } })
  ...
  if (revision.caseStudyId != null) {
    ...
    return node?.solution?.title ? `Case study — ${node.solution.title}` : `Case study #${revision.caseStudyId}`
  }
  return 'this node'
}
