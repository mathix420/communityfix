// Bundled copy of the CommunityFix whitepaper so the MCP `get_whitepaper` tool
// works on Cloudflare Workers (no filesystem at runtime).
//
// SOURCE OF TRUTH: content/whitepaper.md — keep this string byte-for-byte in
// sync with that file. `tests/unit/whitepaper-sync.test.ts` fails if they drift.
export const WHITEPAPER_MARKDOWN = `# Whitepaper
...
export function getWhitepaper() {
  return {
    title: 'CommunityFix Whitepaper',
    url: '/whitepaper',
