...
export default defineTask({
  ...
  async run() {
    ...
    return withScopedDB(async () => {
      ...
      // A pending node is "stuck" when NO terminal moderation outcome exists at
      // or after its last edit. The decision-action guard is what keeps nodes
      // that are *deliberately* pending out of the candidate set: a node flagged
      // uncertain or asked for more info carries a `flag_uncertain` / `request_info`
      // log dated after `updated_at`, so it is left for the human. `updated_at`
      // moves only on a real edit (updateIssue/updateCaseStudy set it; the
      // Workflow's own approve/curate/relocate writes do not), so a node edited
      // after a prior verdict correctly re-qualifies as stuck if its re-trigger
      // failed.
      const stuckIssues = (await db.execute(sql`
      ...
      const stuckCaseStudies = (await db.execute(sql`
        SELECT id FROM case_studies cs
      ...
      // Fire sequentially — these are cheap `Workflow.create()` calls, and serial
      // submission is gentler on the instance-creation rate limit than a burst.
      for (const row of stuckIssues) await triggerModeration('issue', row.id)
      for (const row of stuckCaseStudies) await triggerModeration('case-study', row.id)

      const summary = `Re-triggered ${stuckIssues.length} issue(s), ${stuckCaseStudies.length} case study(ies)`
      if (stuckIssues.length || stuckCaseStudies.length) {
        console.log(
          `[review:reconcile-pending] ${summary} — issues=[${stuckIssues
            .map((r) => r.id)
            .join(',')}] caseStudies=[${stuckCaseStudies.map((r) => r.id).join(',')}]`,
        )
      }
      return {
        result: summary,
        issues: stuckIssues.map((r) => r.id),
        caseStudies: stuckCaseStudies.map((r) => r.id),
      }
    })
  },
})
