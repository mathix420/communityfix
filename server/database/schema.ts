...
export const AUDIT_LOG_ACTIONS = [
  ...
  'propose', 'revise', 'withdraw',
] as const
export type AuditLogAction = typeof AUDIT_LOG_ACTIONS[number]

export const REVISION_STATUSES = ['pending', 'approved', 'rejected', 'withdrawn', 'superseded'] as const
...
export const revisionsRelations = relations(revisions, ({ one }) => ({
  ...
  proposer: one(users, { fields: [revisions.proposerId], references: [users.id], relationName: 'revisionProposer' }),
  decidedBy: one(users, { fields: [revisions.decidedById], references: [users.id], relationName: 'revisionDecidedBy' }),
}))
...
export const nodeMembers = pgTable('node_members', {
  id: serial('id').primaryKey(),
  targetKind: text('target_kind').notNull().$type<RevisionTargetKind>(),
  issueId: integer('issue_id').references(() => issues.id, { onDelete: 'cascade' }),
  caseStudyId: integer('case_study_id').references(() => caseStudies.id, { onDelete: 'cascade' }),
