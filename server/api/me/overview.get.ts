...
export default defineEventHandler(async (event) => {
  ...
  const [profile, issueRows, solutionRows, caseStudyRows, mergedRows, openRows, topicRows] =
    await Promise.all([
      db.query.users.findFirst({
        where: eq(users.id, userId),
        columns: { name: true, trustScore: true, createdAt: true },
      }),
      db
        .select({ n: count() })
        .from(issues)
        .where(
          and(eq(issues.authorId, userId), eq(issues.type, 'issue'), ne(issues.status, 'rejected')),
        ),
      db
        .select({ n: count() })
        .from(issues)
