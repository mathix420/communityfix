...
export default defineEventHandler(async () => {
  ...
  const [uncertain, pendingAppeals, infoReceived, banAppeals, pendingCaseStudies, pendingRevisions] = await Promise.all([
    db.query.auditLogs.findMany({
      ...
      with: {
        issue: {
          columns: {
            id: true, title: true, summary: true, description: true, type: true, status: true,
    ...
    db.query.issues.findMany({
      ...
      columns: {
        id: true, title: true, summary: true, description: true, type: true, status: true,
        rejectionReason: true, appealReason: true, appealedAt: true, createdAt: true,
      },
    ...
    db.query.issues.findMany({
      ...
      columns: {
        id: true, title: true, summary: true, description: true, type: true, status: true,
        infoRequest: true, infoResponse: true, infoRespondedAt: true, createdAt: true,
      },
    ...
    db.query.users.findMany({
      ...
      columns: {
        id: true, name: true, email: true, trustScore: true, createdAt: true,
        banReason: true, banAppealReason: true, banAppealedAt: true, bannedUntil: true,
      },
      limit: 50,
    }),
    ...
    db.query.caseStudies.findMany({
      ...
      columns: {
        ...
        description: true, locationName: true, implementer: true, createdAt: true,
      },
  ...
  for (const row of rejectionCounts) {
    if (row.userId) rejectionMap.set(row.userId, { rejected: Number(row.rejected), approved: Number(row.approved) })
  }
  ...
  return {
    ...
    pendingRevisions: pendingRevisions.map(r => ({
      ...serializeRevision(r),
      node: r.issue
        ? { targetKind: 'issue' as const, issueId: r.issue.id, caseStudyId: null, label: r.issue.title }
        : {
            targetKind: 'case_study' as const,
            issueId: null,
            caseStudyId: r.caseStudy?.id ?? r.caseStudyId,
            label: r.caseStudy?.solution?.title ? `Case study — ${r.caseStudy.solution.title}` : `Case study #${r.caseStudyId}`,
          },
    })),
  }
})
