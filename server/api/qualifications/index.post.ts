...
export default defineEventHandler(async (event) => {
  ...
  if (title.length > MAX_TITLE)
    throw createError({ statusCode: 400, statusMessage: `Title too long (max ${MAX_TITLE})` })
  if (area.length > MAX_AREA)
    throw createError({ statusCode: 400, statusMessage: `Area too long (max ${MAX_AREA})` })
  if (detail && detail.length > MAX_DETAIL)
    throw createError({ statusCode: 400, statusMessage: `Detail too long (max ${MAX_DETAIL})` })
