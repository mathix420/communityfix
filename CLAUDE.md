...
- **Identity (one DID for communityfix.org)**: a single AT Proto account hosted on a PDS provider (default `bsky.social`) custodies the repo signing keys; CommunityFix authenticates with an **app password** stored as a Worker secret (Doppler → `wrangler secret bulk`). `createSession` resolves the DID, which is cached in `standard_site_records`. Running your own PDS works too — just point the env vars at it.
- **AT Protocol client**: `server/utils/atproto.ts` — a tiny `fetch`-based XRPC wrapper (`createSession`, `uploadBlob`, `createRecord`, `putRecord`, `getRecord`, `deleteRecord`) plus a `tid` rkey generator. No `@atproto/api` dependency — it runs unchanged on Workers.
- **Mapping**: the whole site → one `site.standard.publication` record (`url` = `https://communityfix.org`, `name` = "CommunityFix", plus `icon`, `description`, `basicTheme` = brand RGB, `preferences.showInDiscover`). Each **publishable** issue/solution/case study → a `site.standard.document` (`site` = publication AT-URI; `path` = `/issue/:id` or `/case-study/:id`; `title`, `publishedAt` required; `description`/`textContent`/`tags`/`updatedAt` recommended). `contributors` lists only authors who have linked an AT Proto DID (`users.atproto_did`); civic users without one are omitted. CommunityFix's tree linkage + SDG mapping ride along in the open-union `content` field under `$type: org.communityfix.content`.
- **Publishability + publish flag**: a node is mirrored iff `approved && !isSpam && publishToStandardSite` (`publish_to_standard_site` column on `issues` + `case_studies`, default true, admin-clearable to opt a node out without rejecting it).
- **Domain logic**: `server/utils/standard-site.ts` builds the record bodies, publishes idempotently (first publish via `createRecord`, updates reuse the stored rkey via `putRecord` guarded by the last CID, unchanged content skipped via a sha256 hash, transient Worker failures retried with backoff), and mirrors AT-URIs into `standard_site_records` (migrations `0015`–`0017`).
- **Sync (two paths)**:
  - *Per-node, event-driven*: `reconcileNode(refKind, refId)` / `reconcileNodeInBackground(...)` publish/update/delete a single node's document. Wired into every publishability transition — AI + admin approve/reject, appeal outcomes, and content/metadata edits (`review-issue`, `review-case-study`, the admin endpoints, and the shared `issue-write`/`case-study-write` update fns). Fire-and-forget so a PDS hiccup never breaks the user's request.
  - *Full reconcile*: scheduled task `sync:standard-site` (daily 4am UTC) or admin-only `POST /api/admin/standard-site/sync`. Republishes the publication + all publishable documents and **prunes** records whose node is no longer publishable — the backstop that repairs drift from any failed per-node write. Both no-op cleanly when credentials are unset.
- **Web verification** (bidirectional domain↔record proof):
  ...
  - The app shell (`app.vue`) emits the optional `<link rel="site.standard.publication" href="at://…">` discovery hint site-wide, fed by `GET /api/standard-site/publication`.
