# LinkedIn Outreach — Lachlan Fahy

**Profile:** https://www.linkedin.com/in/lachlanfahy/

---

## Connection request note (≤300 characters)

Hi Lachlan — I'm Arnaud, founder of CommunityFix. I've documented TerraCool as a case study on the platform and would love your eyes on it for accuracy. Happy to connect!

---

## Follow-up message (after connection accepted)

Hi Lachlan,

Thanks for connecting. I wanted to reach out about TerraCool — your ceramic evaporative cooling module project from the Bartlett.

I run CommunityFix (communityfix.org), a public knowledge base that documents real-world attempts to address urban and environmental challenges. We include projects with inconclusive or in-progress outcomes, not just clear-cut successes — the process and the lessons are what matter to practitioners trying to build on existing work.

I've added a case study entry for TerraCool (#64). I'd really value you (and Dilara, if she's up for it) taking a look to:

- confirm the description is accurate
- add anything you think would be useful — fabrication notes, material sourcing details, links to open-source files, or what you'd approach differently

The entry is here: https://communityfix.org/case-studies/64

No pressure on timing — I know you're likely deep in other projects. But if there's a moment to review it, even briefly, it would make the resource much more useful for others working on passive cooling in urban contexts.

Thanks for building something worth documenting.

Arnaud Gissinger
Founder — CommunityFix · communityfix.org
