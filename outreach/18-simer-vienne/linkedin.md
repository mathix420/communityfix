# LinkedIn — SIMER

Profil : https://fr.linkedin.com/company/simer86

---

## Note de mise en relation (≤ 300 caractères)

> Bonjour, je suis le fondateur de CommunityFix, une plateforme qui documente des initiatives environnementales concrètes. Votre programme R'œuf'cyclage y figure comme étude de cas. J'aimerais vous en informer et échanger si vous le souhaitez.

*(253 caractères)*

---

## Message après connexion

Bonjour,

Merci d'avoir accepté la connexion.

Je voulais vous informer que le programme R'œuf'cyclage du SIMER — le suivi et la distribution d'environ 800 poules pondeuses dans le cadre d'un plan ADEME — est documenté comme étude de cas de référence sur CommunityFix (communityfix.org).

L'objectif de la plateforme est de permettre à d'autres territoires et organisations de s'inspirer d'initiatives qui ont fait leurs preuves. Votre démarche en fait clairement partie.

En documentant ce qui a réellement été tenté — coûts et écueils compris —, d'autres collectivités peuvent éviter les mêmes erreurs et aller droit aux solutions qui fonctionnent. C'est une contribution concrète à un monde meilleur : transformer l'expérience d'une collectivité en un savoir utile à tous.

Si vous souhaitez vérifier l'exactitude de la fiche ou l'enrichir (chiffres récents, retours d'expérience, photos), je suis à votre disposition.

Bien cordialement,
**Arnaud Gissinger**
Fondateur — CommunityFix · communityfix.org
