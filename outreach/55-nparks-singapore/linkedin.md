# LinkedIn Outreach — NParks Singapore (Case Study 55)

**Profile:** https://sg.linkedin.com/company/nationalparksboardsg

---

## Connection note (≤300 characters)

Hi NParks team — we've documented Singapore's City in Nature programme (Skyrise Greenery scheme + greenery replacement mandate) as a reference case study on CommunityFix. Would love your eyes on it for accuracy. — Arnaud, Founder · communityfix.org

---

## Follow-up message (after connection accepted)

Hi NParks team,

Thank you for connecting.

As mentioned, CommunityFix (communityfix.org) is a free public platform that maps real-world solutions to urban and environmental challenges. We've published Singapore's "City in Nature" initiative as a reference case study — specifically the combination of the mandatory greenery replacement requirement and the Skyrise Greenery Incentive Scheme.

We chose it because the policy design is genuinely distinctive: binding obligations on developers, paired with financial incentives that make compliance economically viable. It's the kind of integrated approach many cities are trying to replicate but rarely get right.

The idea behind CommunityFix is simple: by documenting what was actually tried — and what came of it, costs and pitfalls included — other cities, regions and countries can avoid the same mistakes and go straight to the solutions that work. It's a small but real contribution to a better world: turning one community's hard-won experience into knowledge everyone can build on.

We'd welcome a quick review from your side to make sure all the details are accurate. And if anyone on the team would like to add metrics, updated programme data, or lessons learned, that would make the record even more valuable for policymakers looking to adapt the model.

Case study: https://communityfix.org (search "City in Nature Singapore")

Happy to share the direct link or answer any questions about the platform.

Thank you for the work NParks does — it's a genuine model for urban greening.

Best,
Arnaud Gissinger
Founder — CommunityFix · communityfix.org
