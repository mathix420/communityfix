# Message — Formulaire de contact Siredom

**URL formulaire :** https://www.siredom.com (page contact)
**Objet suggéré :** Votre opération « 200 poules » documentée sur CommunityFix

---

Bonjour,

Je me permets de vous contacter au nom de CommunityFix (communityfix.org), une plateforme collaborative qui recense des solutions concrètes à des défis environnementaux et urbains.

L'opération annuelle « 200 poules » menée à Lisses en partenariat avec la Commune a été documentée comme étude de cas de référence sur notre plateforme. Elle illustre notamment une leçon précieuse sur le calendrier de distribution, utile pour d'autres collectivités souhaitant mettre en œuvre un programme similaire.

Nous souhaiterions vous inviter à :
1. **Vérifier l'exactitude** des informations publiées ;
2. **Enrichir la fiche** si vous disposez de données complémentaires (métriques, retours d'expérience, sources).

L'idée derrière CommunityFix est simple : en documentant ce qui a réellement été tenté — et ce que cela a donné, coûts et écueils compris —, d'autres villes, régions et pays peuvent éviter les mêmes erreurs et aller droit aux solutions qui fonctionnent. C'est une contribution modeste mais concrète à un monde meilleur : transformer l'expérience durement acquise d'une collectivité en un savoir utile à tous.

Vous pouvez consulter l'étude de cas directement sur communityfix.org. N'hésitez pas à nous répondre via ce formulaire ou à nous contacter à l'adresse indiquée sur le site.

Cordialement,

Arnaud Gissinger
Fondateur, CommunityFix
communityfix.org
