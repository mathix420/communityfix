...
function summaryLine(log: LogRow): string {
  ...
  switch (log.action) {
    case 'approve':
      return `${issue} approved — ${truncate(log.reason, 80)}`
    case 'reject':
      return `${issue} rejected — ${truncate(log.reason, 80)}`
    case 'flag_spam':
      return `${issue} flagged as spam`
    case 'flag_duplicate':
      return `${issue} duplicate of #${(d.duplicateOfId as number | undefined) ?? (d.targetId as number | undefined) ?? '?'}`
    case 'reparent':
      return `${issue} moved under #${d.targetId as number | undefined}`
    case 'convert_to_case_study':
      return `${issue} should be a case study of #${d.targetId as number | undefined}`
    case 'ban':
      return `${who} auto-banned (${d.rejectedCount as number | undefined}/${d.lookbackWindow as number | undefined} rejected)`
    case 'unban':
      return `${who} unbanned`
    case 'score_update':
      return `${who}: ${d.oldScore as number | undefined} → ${d.newScore as number | undefined}`
    case 'appeal_submitted':
      return `${issue} appeal by ${who}`
    case 'appeal_approved':
      return `${issue} appeal granted`
    case 'appeal_denied':
      return `${issue} appeal denied`
    case 'override_approve':
      return `${issue} force-approved (was ${d.previousStatus as string | undefined})`
    case 'override_reject':
      return `${issue} force-rejected (was ${d.previousStatus as string | undefined})`
    case 'remod':
      return `${issue || (d.caseStudyId ? `case study #${d.caseStudyId}` : '')} re-moderation triggered (was ${d.previousStatus as string | undefined})`
    case 'flag_uncertain':
      return `${issue} uncertain (${Math.round(((d.confidence as number | undefined) ?? 0) * 100)}% confidence) — ${truncate(log.reason, 60)}`
    case 'request_info':
      return `${issue} info requested — ${truncate(log.reason, 60)}`
    default:
      return truncate(log.reason || '', 80)
  }
}
...
watch([typeFilter, statusFilter, fromDate, toDate, limit], () => {
  page.value = 1
})
...
<template>
  <div class="space-y-4">
      ...
      <div class="space-y-2">
        <div class="flex flex-wrap gap-2 items-center">
          ...
          <USelectMenu
            ...
            :items="statusOptions"
          />
          <USelectMenu v-model="limit" class="w-32" value-key="value" :items="limitOptions" />
        </div>
        <div class="flex flex-wrap gap-2 items-center">
          <UInput v-model="issueIdFilter" class="w-32" placeholder="Issue ID" type="number" />
          <UInput v-model="userIdFilter" class="w-72" placeholder="User ID (uuid)" />
    ...
    <UiCard v-if="logs.length" padding="none">
      <div class="divide-y divide-gray-100">
        <div v-for="log in logs" :key="log.id">
          ...
          >
            <span
              class="text-[11px] text-toned tabular-nums w-16 shrink-0"
              :title="formatTimeFull(log.createdAt)"
            >
              {{ formatTime(log.createdAt) }}
            </span>
            <UiBadge class="shrink-0" :variant="actionVariant[log.action] ?? 'default'">
              {{ actionLabel[log.action] ?? log.action }}
            </UiBadge>
            <span class="text-sm text-gray-700 truncate min-w-0 flex-1">
              {{ summaryLine(log) }}
            </span>
            <UiBadge class="shrink-0" :variant="statusVariant[log.status] ?? 'default'">
          ...
          <div
            ...
            class="bg-gray-50/80 px-4 pb-4 pt-2 space-y-3 text-sm border-b border-gray-200"
          >
            <p v-if="log.reason" class="text-gray-600 leading-relaxed">
              {{ log.reason }}
            </p>
            <div
              v-if="(log.details?.confidence as number | undefined) != null"
              class="flex items-center gap-2 text-xs"
            >
              <span
                class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 font-medium"
                :class="(log.details!.confidence as number) >= 0.7
                  ? 'bg-primary-50 text-primary-700'
                  : 'bg-gray-100 text-gray-700'"
              >
                <UIcon class="size-3" name="lucide:gauge" />
                {{ Math.round((log.details!.confidence as number) * 100) }}% confidence
              </span>
            ...
            <div v-if="log.details" class="flex flex-wrap gap-1.5">
              ...
              >
                <UIcon class="size-3" name="lucide:copy" />
                Duplicate of #{{ log.details.duplicateOfId }}
              </NuxtLink>
              <NuxtLink
              ...
              >
                <UIcon class="size-3" name="lucide:trending-up" />
                {{ log.details.oldScore }} → {{ log.details.newScore }}
              </span>
              <span
            ...
            <div
              v-if="(log.details?.similarIssues as Array<{ id: number; similarity: number }> | undefined)?.length"
              class="text-xs text-toned"
            >
              Similar:
              <span
                v-for="(s, i) in (log.details!.similarIssues as Array<{ id: number, similarity: number }>)"
                :key="i"
              >
                {{ i > 0 ? ', ' : '' }}
            ...
            <div
              v-if="log.reviewer || log.reviewNote"
              class="text-xs text-toned border-l-2 border-l-gray-300 pl-2"
            >
              <span v-if="log.reviewer">
                {{ log.reviewer.name }}
              </span>
              <span v-if="log.reviewNote">
                — {{ log.reviewNote }}
              </span>
            </div>
            <div class="flex items-center gap-3 text-xs text-toned">
              <span>
                {{ formatTimeFull(log.createdAt) }}
              </span>
              <span>
            ...
            <details class="text-xs">
              <summary class="text-toned cursor-pointer">
                Raw JSON
              </summary>
            ...
            <div class="flex flex-wrap items-center gap-2 pt-2 border-t border-gray-200">
              <template v-if="log.status === 'needs_review'">
                <UInput v-model="reviewNote" class="w-48" placeholder="Admin note..." size="xs" />
                <UButton
                  color="neutral"
                  size="xs"
                  variant="outline"
              ...
              <template
                v-if="log.issueId &&
                  (log.action === 'reject' ||
                    log.action === 'flag_spam' ||
                    log.action === 'flag_duplicate' ||
                    log.action === 'convert_to_case_study')"
              >
                <UButton
                  color="primary"
                  size="xs"
              ...
              <template v-if="log.issueId && log.action === 'approve'">
                <UButton color="neutral" size="xs" variant="outline" @click="askReject(log.issueId)">
                  Force Reject
                </UButton>
              </template>
    ...
    <div v-if="totalPages > 1" class="flex justify-center items-center gap-2 flex-wrap">
      ...
      />
      <UButton size="xs" variant="outline" :disabled="!jumpPage" @click="jumpToPage">
        Go
      </UButton>
    </div>
    <AdminRejectModal
