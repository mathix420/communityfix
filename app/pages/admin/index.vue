...
function revisionLink(rev: {
  node: { targetKind: 'issue' | 'case_study'; issueId: number | null; caseStudyId: number | null }
}) {
  ...
  <div class="space-y-8">
    ...
    <section v-if="queue?.pendingRevisions?.length">
      <div class="mb-3 flex items-baseline gap-2">
        ...
        <span class="font-mono text-[10px] text-gray-400 tracking-widest">
          · {{ queue.pendingRevisions.length }}
        </span>
      </div>
      <div class="space-y-2">
        <AdminQueueCard v-for="rev in queue.pendingRevisions" :key="rev.id" :since="rev.createdAt">
          <template #header>
            <div class="flex items-center gap-2 flex-wrap">
              <NuxtLink class="font-medium hover:underline truncate" :to="revisionLink(rev)">
                {{ rev.node.label }}
              </NuxtLink>
              <UiBadge>
                {{ rev.node.targetKind === 'issue' ? 'issue' : 'case study' }}
              </UiBadge>
              <UiBadge
              ...
              >
                <UIcon class="inline size-3" name="lucide:flag" />
                {{ rev.aiVerdict }}{{ rev.aiConfidence != null ? ` ${formatConfidence(rev.aiConfidence)}` : '' }}
              </UiBadge>
              <UiBadge v-else-if="rev.aiVerdict === 'ok'" variant="success">
                AI: ok{{ rev.aiConfidence != null ? ` ${formatConfidence(rev.aiConfidence)}` : '' }}
              </UiBadge>
              <AdminSlaBadge hide-when-fresh :since="rev.createdAt" />
            </div>
            <p v-if="rev.note" class="text-sm text-toned line-clamp-2">
              {{ rev.note }}
            </p>
            <AdminAuthorBadge
              v-if="rev.proposer"
          ...
          <template #meta>
            <div
              v-if="rev.aiReason && rev.aiVerdict && rev.aiVerdict !== 'ok'"
