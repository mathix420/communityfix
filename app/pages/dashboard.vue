...
const firstName = computed(() => (o.value.name || user.value?.name || '').trim().split(/\s+/)[0] || null)
...
const memberSince = computed(() =>
  o.value.memberSince ? new Date(o.value.memberSince).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : null,
)
...
interface Activity { id: string, kind: ActivityKind, label: string, at: string, link: string, who?: string }

const activity = computed<Activity[]>(() => {
  const events: Activity[] = []
  for (const e of mine.value) {
    events.push({ id: `m${e.id}p`, kind: 'proposed', label: e.node.label, at: e.createdAt, link: nodeLink(e) })
    if (e.decidedAt && (e.status === 'approved' || e.status === 'rejected')) {
      events.push({ id: `m${e.id}d`, kind: e.status, label: e.node.label, at: e.decidedAt, link: nodeLink(e) })
    }
  }
  for (const e of toReview.value) {
    events.push({ id: `r${e.id}`, kind: 'incoming', label: e.node.label, at: e.createdAt, link: nodeLink(e), who: e.proposer?.name || 'Someone' })
  }
...
const activityMeta: Record<ActivityKind, { icon: string, ring: string, tint: string }> = {
  incoming: { icon: 'lucide:git-pull-request-arrow', ring: 'bg-blue-50 text-blue-600', tint: 'text-blue-600' },
  proposed: { icon: 'lucide:pencil-line', ring: 'bg-gray-100 text-gray-500', tint: 'text-gray-500' },
...
function activityVerb(a: Activity) {
  switch (a.kind) {
    case 'incoming': return `${a.who} suggested an edit`
    case 'proposed': return 'You proposed an edit'
    case 'approved': return 'Your edit was approved'
    case 'rejected': return 'Your edit was declined'
    case 'withdrawn': return 'You withdrew an edit'
...
useSeoMeta({
  ...
  description: 'Your contributions, the suggestions awaiting your review, and the edits you have proposed.',
})
    ...
    <div class="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 lg:auto-rows-[minmax(10.5rem,1fr)]">
      ...
      >
        ...
        <div class="relative mt-5 flex flex-wrap gap-2">
          <span class="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 font-mono text-xs text-gray-600 ring-1 ring-gray-200">
            <UIcon name="lucide:badge-check" class="size-3.5 text-primary-600" />
            Trust {{ o.trustScore }}
          </span>
          <span v-if="memberSince" class="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 font-mono text-xs text-gray-600 ring-1 ring-gray-200">
            <UIcon name="lucide:calendar" class="size-3.5 text-gray-400" />
            Since {{ memberSince }}
          </span>
          <span v-if="o.openProposals" class="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1 font-mono text-xs text-gray-600 ring-1 ring-gray-200">
            <UIcon name="lucide:loader" class="size-3.5 text-gray-400" />
      ...
      >
        <div class="flex items-center justify-between">
          <span class="font-mono text-[11px] uppercase tracking-[0.18em] text-gray-300">{{ pad(i + 1) }}</span>
          <UIcon
            v-if="s.to"
      ...
      >
        <div class="mb-4 flex items-center justify-between">
          <h2 class="flex items-center gap-2 font-mono text-sm uppercase tracking-widest text-neutral-900">
            <UIcon name="lucide:scale" class="size-4 text-primary-600" />
            Needs your review
          </h2>
          <span
        ...
        <ul v-if="toReview.length" class="-mr-2 flex max-h-80 flex-col divide-y divide-gray-100 overflow-y-auto pr-2 scrollbar-hide">
          <li v-for="entry in toReview" :key="entry.id">
            ...
            >
              <span class="mt-0.5 inline-flex size-7 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600">
                <UIcon name="lucide:git-pull-request-arrow" class="size-3.5" />
              </span>
              ...
              <UIcon name="lucide:arrow-right" class="mt-1.5 size-4 shrink-0 text-gray-300 transition-colors group-hover:text-primary-500" />
            </NuxtLink>
          </li>
        </ul>

        <div v-else class="flex flex-1 flex-col items-center justify-center py-8 text-center">
          <UIcon name="lucide:inbox" class="size-7 text-gray-300" />
          <p class="mt-2 text-sm text-gray-500">All clear — nothing waiting on you.</p>
        </div>
      </section>
      ...
      >
        <h2 class="mb-4 flex items-center gap-2 font-mono text-sm uppercase tracking-widest text-neutral-900">
          <UIcon name="lucide:activity" class="size-4 text-primary-600" />
          Recent activity
        </h2>

        <ul v-if="activity.length" class="-mr-2 flex max-h-80 flex-col divide-y divide-gray-100 overflow-y-auto pr-2 scrollbar-hide">
          <li v-for="a in activity" :key="a.id">
            <NuxtLink :to="a.link" class="group flex items-start gap-3 py-3">
              <span
                class="mt-0.5 inline-flex size-7 shrink-0 items-center justify-center rounded-full"
                :class="activityMeta[a.kind].ring"
              >
                <UIcon :name="activityMeta[a.kind].icon" class="size-3.5" />
              </span>
              <span class="min-w-0 flex-1">
                <span class="block text-sm text-neutral-700">
                  <span :class="activityMeta[a.kind].tint">{{ activityVerb(a) }}</span>
                </span>
                <span class="block truncate text-xs text-gray-500 group-hover:text-primary-600">
                  {{ a.label }}
                </span>
              </span>
              <span class="mt-0.5 shrink-0 font-mono text-[11px] text-gray-400">{{ formatRelative(a.at) }}</span>
            </NuxtLink>
          </li>
        </ul>

        <div v-else class="flex flex-1 flex-col items-center justify-center py-8 text-center">
          <UIcon name="lucide:wind" class="size-7 text-gray-300" />
      ...
      >
        <div class="mb-4 flex items-baseline gap-3">
          <h2 class="flex items-center gap-2 font-mono text-sm uppercase tracking-widest text-neutral-900">
            <UIcon name="lucide:hash" class="size-4 text-primary-600" />
            Your topics
          </h2>
          <span class="font-mono text-[11px] text-gray-400">tags you contribute to</span>
        </div>
        ...
        <div v-if="o.topics.length" class="flex flex-wrap gap-2">
          ...
          >
            <span class="text-gray-700 group-hover:text-primary-700">
              <span class="text-gray-400">#</span>{{ t.slug }}
            </span>
        ...
        <div v-else class="flex items-center gap-3 text-sm text-gray-500">
          <UIcon name="lucide:compass" class="size-5 text-gray-300" />
          <span>No topics yet —
            <NuxtLink to="/new" class="text-primary-600 hover:underline" @click="track('Dashboard start issue')">start an issue</NuxtLink>
            to stake out your areas.</span>
        </div>
      </section>
    </div>
  </AppContainer>
...
.tile {
  position: relative;
  animation: tile-rise 0.55s cubic-bezier(0.2, 0.7, 0.2, 1) backwards;
}
