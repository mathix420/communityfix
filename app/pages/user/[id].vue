...
<template>
  <AppContainer class="container overflow-x-clip h-fit mx-auto p-4">
    <div
      v-if="user"
      class="w-full my-28 gap-4 sm:gap-6 text-center flex flex-col items-center justify-center"
    >
      <img
    ...
    <div v-if="user && user.issues.length > 0">
    ...
    <div v-if="user && user.solutions.length > 0" class="mt-12">
      <p class="text-center text-lg sm:text-xl font-title text-primary-950 mb-8">
    ...
    <div
      v-else-if="user && user.issues.length === 0"
