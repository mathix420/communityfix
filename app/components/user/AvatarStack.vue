...
const ringClass = computed(() => (props.ring === 'muted' ? 'ring-gray-50' : 'ring-white'))
...
<template>
  <div class="flex items-center shrink-0">
    <component
      :is="linked && p.id ? 'NuxtLink' : 'span'"
      v-for="(p, i) in shown"
      :key="p.id || p.name"
      class="relative -ml-2 first:ml-0 inline-flex rounded-full ring-2 bg-white transition-transform hover:-translate-y-0.5 hover:z-10"
      :class="ringClass"
      :style="{ zIndex: shown.length - i }"
      :title="tooltip(p, i)"
      :to="linked && p.id ? `/user/${p.id}` : undefined"
    >
      <img class="size-6 rounded-full bg-gray-100" loading="lazy" :alt="p.name" :src="avatarUrl(p)">
    </component>
    <span
      v-if="overflow"
      class="relative -ml-2 inline-flex items-center justify-center size-6 rounded-full ring-2 bg-gray-100 text-[10px] font-mono text-gray-600"
      :class="ringClass"
      :title="`${overflow} more`"
    >
      +{{ overflow }}
